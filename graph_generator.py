...
root_dir = 'C:\\Users\\bugl2301\\Documents\\new_graphs'
# Generate graphs


# region_range = np.arange(start=start_interval - step,
#                          stop=end_interval + step, step=step)
# date_range = np.arange(start=start_interval - step, stop=2 + step, step=step)
# sociability_range = np.arange(
#     start=start_interval, stop=end_interval + step, step=step)
...
for i in iterations:
    for sd in ['rand', 'exp']:
        for region_bias in np.arange(start=0,
                                     stop=1.1, step=0.1):
            for date_bias in [round(x * 0.1, 1) for x in range(-10, 11)]:
                for sociability_bias in np.arange(start=2, stop=3.1, step=0.1):
                    sb = sociability_bias
                    if sd == 'rand':
                        sb = np.random.uniform(1, 10)

                    print(
                        f'count-{count}-it={i}-sd={sd}-region={region_bias}-date={date_bias}-sociability={sb}')
                    cmd = f"node graph_generator/src/index.js -o {root_dir}\\iter-{i}_r-{region_bias}_sd-{sd}_sp-{sb}_d-{date_bias} -u 632 -t 6864 -sd {sd} -sp {sb} -r {region_bias} -d {date_bias}"
                    os.system(cmd)
                    count += 1
