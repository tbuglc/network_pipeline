...
outpath_raw = './new_filters'
mkdirs(outpath_raw)

# output folder will be created if it does not exist
cmd = f'python graph_parser/main.py -i=./data/raw -o={outpath_raw}'
print(cmd)
os.system(cmd)

## filter ##

outpath_acc = f'./new_filters/acc{accorderie_id}'
mkdirs(outpath_acc)
...
cmd = f'python graph_filter/main.py -i={outpath_raw} -o={outpath_acc} --accorderie_edge={accorderie_id}'
print(cmd)
os.system(cmd)
...
outpath_xlsx = f'./new_filters/acc{accorderie_id}/data.xlsx'
...
for group_name in groups:
    ...
    for filter_name in groups[group_name]:
        ...
        outpath_filtered_xlsx = f'./new_filters/acc{accorderie_id}/{group_name}/data_{filter_name}.xlsx'
    ...
    inpath_plots = f'./new_filters/acc{accorderie_id}/{group_name}'
    outpath_plots = f'./new_filters/acc{accorderie_id}/{group_name}/pdf/data.pdf'
    ...
    # filter by date
    # -i={outpath_raw} -o={outpath_acc} --accorderie_edge={accorderie_id}
